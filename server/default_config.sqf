...
A3W_extDB_Environment = "a3w2";    // Value used to separate player & object data from multiple environments running on the same map (e.g. "normal", "hardcore", "dev", etc. can be whatever you want)
...
A3W_vehicleQuantity = 160;         // Approximate number of land vehicles to be spawned in towns
...
A3W_sideMissionTimeout = 35*60;    // Time in seconds that a Side Mission will run for, unless completed
